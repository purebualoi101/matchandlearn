...
def user_profile(request,user_id):
    ...
    if request.POST.get('subject_good'):
        ...
        user_add.save()
        return render(request, 'tinder/your_subject.html', {'comments': comments,'pic':pic,'name': Userinfo.objects.get(id=user_id),'subject': Userinfo.objects.get(name=request.user.username).expert_subject.all()})
    return render(request,'tinder/your_subject.html', {'comments': comments,'pic':pic,'name': Userinfo.objects.get(id=user_id),'subject': Userinfo.objects.get(name=request.user.username).expert_subject.all(),'test':Userinfo.objects.get(name=request.user.username).match.all()})
...
def another_profile(request,user_id):
    ...
    if request.POST.get('comment_input'):
        ...
        if not Comment.objects.filter(whocomment = Username, commentto= match_guy):
            create_comment = Comment.objects.create(comment_value = comment_text, whocomment = Username, commentto= match_guy)
            create_comment.save()
    ...
    return render(request,'tinder/profile.html',{'comments': comments,'pic':pic,'profile': modelget,'subject':modelget.expert_subject.all(),'name': Userinfo.objects.get(name =request.user.username),"chat_room_name":url_chat})
...
def home_page(request):
    ...
    if request.POST.get('subject_find'):
        ...
        # user use school name and gender to find tutor.
            ...
            for key in tutor_list:
                result[key] = Profilepic.objects.get(user=key)
        return render(request, 'tinder/home.html', {'infoma':result,'name':Userinfo.objects.get(name=request.user.username),"search_result": tutor_list, "search_size": len(tutor_list),'sendPOST' : sendPOST, "what_sub": request.POST['subject_find']})
    close_old_connections()
    db.connection.close()
    return render(request,'tinder/home.html',{ 'name':Userinfo.objects.get(name=request.user.username), "search_size": len(tutor_list),'sendPOST':sendPOST,'test':Userinfo.objects.get(name=request.user.username).request.all()})
